...
const updateUserCoverImage = asyncHandler(async (req: Request, res) => {
...
const clearWatchHistory = asyncHandler(async (req, res) => {
    ...
    return res.json(
        new ApiResponse(
            200,
            { cleared: true },
            "Successfully cleared watch history"
        )
    );
});
...
const playPauseWatchHistory = asyncHandler(async (req, res) => {
    ...
    return res.json(
        new ApiResponse(
            200,
            { isHistory: user.isHistory },
            `Successfully ${status} watch history`
        )
    );
});
...
const toggleWatchLater = asyncHandler(async (req, res) => {
    ...
    if (user.watchLater.includes(new Types.ObjectId(videoId))) {
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                response,
                "Successfully fetched watch later videos"
            )
        );
});
