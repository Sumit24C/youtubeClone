import {
    List,
    ListItem,
    ListItemButton,
    ListItemText,
    Checkbox,
    Divider,
    Paper,
} from "@mui/material";
import { useState, useEffect } from "react";
import { isCancel } from "axios";
import { useAxiosPrivate } from "../../hooks/useAxiosPrivate";
import { extractErrorMsg } from "../../utils/extractErrorMsg";
import { useSnackbar } from "notistack";
import type { Playlist } from "../../types/playlist";

type Props = {
    playlist: Playlist[];
    videoId?: string;
};

function PlaylistSelector({ playlist, videoId }: Props) {
    const [selectedPlaylist, setSelectedPlaylist] = useState<string[]>([]);
    const [errMsg, setErrMsg] = useState<string>("");
    const [loading, setLoading] = useState<boolean>(false);

    const axiosPrivate = useAxiosPrivate();
    const { enqueueSnackbar } = useSnackbar();

    useEffect(() => {
        if (!videoId) return;

        const initiallySelectedId = playlist
            .filter((pl) => pl.videos?.includes(videoId))
            .map((pl) => pl._id);

        setSelectedPlaylist(initiallySelectedId);
    }, [playlist, videoId]);

    const handleToggle = (p_id: string): void => {
        setSelectedPlaylist((prev) =>
            prev.includes(p_id)
                ? prev.filter((p) => p !== p_id)
                : [...prev, p_id]
        );
    };

    const toggleVideo = async (pId: string): Promise<void> => {
        if (!videoId) return;

        setLoading(true);
        setErrMsg("");

        try {
            const response = await axiosPrivate.patch(
                `/playlist/toggle/${videoId}/${pId}`,
                {}
            );

            enqueueSnackbar(response.data.message);
            handleToggle(pId);
        } catch (error: any) {
            if (isCancel(error)) {
                console.error("selectedPlaylistAxios :: error :: ", error);
            } else {
                const errorMessage = extractErrorMsg(error);
                setErrMsg(errorMessage);
                enqueueSnackbar(errorMessage);
            }
        } finally {
            setLoading(false);
        }
    };

    return (
        <Paper
            elevation={2}
            sx={{
                width: "100%",
                maxHeight: 250,
                overflowY: "auto",
                bgcolor: "#000000ff",
                borderRadius: 1,
                my: 1,
                px: 1,
                py: 0.5,
            }}
        >
            {errMsg && <p style={{ color: "red" }}>{errMsg}</p>}

            <List sx={{ width: "100%", maxHeight: 300 }}>
                {playlist.map((pl) => (
                    <div key={pl._id}>
                        <ListItem disablePadding>
                            <ListItemButton
                                disabled={loading}
                                onClick={() => toggleVideo(pl._id)}
                            >
                                <Checkbox
                                    edge="start"
                                    checked={selectedPlaylist.includes(pl._id)}
                                    tabIndex={-1}
                                    disableRipple
                                />
                                <ListItemText primary={pl.name} />
                            </ListItemButton>
                        </ListItem>
                        <Divider />
                    </div>
                ))}
            </List>
        </Paper>
    );
}

export default PlaylistSelector;